...
public class CookieTestController {
	...
	public String test1(HttpServletResponse response) {
		...
		response.addCookie(new Cookie("fid", "ABCDEF"));
		response.addCookie(new Cookie("cid", "GHIJK"));
	...
	public String test3(
			@CookieValue(name = "uid", required = true) Cookie uidCookie) {
		log.info("test3에서 받은 쿠기: {}",
				uidCookie.getName(), uidCookie.getValue());
		return "/cookie/index";	
	}
