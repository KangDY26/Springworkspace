...
public class ResponseEntityTestController {
	...
	// jacson-databind 가 ResponseEntity의 Body 내용도 JSON 형식으로 변환해준다
	@GetMapping("/test2")
	...
	public ResponseEntity<String> test3(FruitDTO fruitDTO) {
		...
		return ResponseEntity.ok()
				.contentType(MediaType.TEXT_PLAIN).body("오케이!");
	}
	...
	public ResponseEntity<String> test4(
		...
		if (updateResult) {
			return ResponseEntity.ok()
					.contentType(MediaType.TEXT_PLAIN).body("4번 텍스트 통과!");
		} else {
			return ResponseEntity.status(502)
					.contentType(MediaType.TEXT_PLAIN).body("4번 텍스트 실패!");
		}
